...
class Interpreter:
    ...
    """
    def __init__(self, code, input_args=None):
        """
        Initialize a new interpreter.

        Arguments:
            code -- the code to execute as a string
            input_args -- list of string arguments passed via command line
        """
        ...
        # setup CLI input buffer if arguments were provided
        self._input_args = input_args
        self._input_buffer = []
        if self._input_args is not None:
            for arg in self._input_args:
                for char in arg:
                    self._input_buffer.append(ord(char))
                self._input_buffer.append(0)  # null-byte separator
    ...
    def _input(self):
        """
        Return an inputted character or ASCII byte code.
        """
        if self._input_args is not None:
...
if __name__ == "__main__":
    ...
    # Separate arguments after '--' for CLI input buffering
    raw_sys_args = sys.argv[1:]
    input_args = None
    if "--" in raw_sys_args:
        dash_idx = raw_sys_args.index("--")
        fish_sys_args = raw_sys_args[:dash_idx]
        input_args = raw_sys_args[dash_idx + 1:]
    else:
        fish_sys_args = raw_sys_args
    ...
    # parse arguments from fish_sys_args
    arguments = parser.parse_args(fish_sys_args)
    ...
    interpreter = Interpreter(code, input_args=input_args)
